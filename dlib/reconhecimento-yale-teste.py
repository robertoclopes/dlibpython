...
import cv2
import numpy as np
from PIL import Image   #lib para leitura dos arquivos .gif
...
limiar = 0.5    #distancia <= 0,5 para fazer a classificacao
totalFaces = 0
totalAcertos = 0
...
for arquivo in glob.glob(os.path.join("yalefaces/teste", "*.gif")):
    ...
    idatual = int(os.path.split(arquivo)[1].split(".")[0].replace("subject", ""))   #pega os ids das imagens 01, 02, 03
    totalFaces += 1
    ...
    for face in facesDetectadas:
        ...
        #conversoes dos tipos para classificacao
        ...
        if distanciaMinima <= limiar:
            ...
            if idprevisto == idatual:   #compara os ids para saber se foi classificado corretamente ou nao
                totalAcertos += 1
        else:
            nome = 'Desconhecido'

        print("idatual: {} idprevisto: {}".format(idatual, idprevisto)) #resposta correta e previsao
...
percentualAcerto = (totalAcertos / totalFaces) * 100    #percentual de acertos
